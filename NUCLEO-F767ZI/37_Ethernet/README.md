...
**Checksum 탭 → Infrastructure - Checksum Options**

> 💡 **STM32F7는 하드웨어 체크섬을 지원**하므로, 소프트웨어 체크섬을 **Disabled**로 설정하여 CPU 부하를 줄입니다.

| 파라미터 | 값 | 설명 |
|----------|-----|------|
| **CHECKSUM_BY_HARDWARE** | **Enabled** | 하드웨어 체크섬 사용 ⭐ |
| LWIP_CHECKSUM_CTRL_PER_NETIF | Disabled | |
| CHECKSUM_GEN_IP | Disabled (0) | SW 생성 비활성화 |
| CHECKSUM_GEN_UDP | Disabled (0) | SW 생성 비활성화 |
| CHECKSUM_GEN_TCP | Disabled (0) | SW 생성 비활성화 |
| CHECKSUM_GEN_ICMP | Disabled (0) | SW 생성 비활성화 |
| CHECKSUM_CHECK_IP | Disabled (0) | SW 검증 비활성화 |
| CHECKSUM_CHECK_UDP | Disabled (0) | SW 검증 비활성화 |
| CHECKSUM_CHECK_TCP | Disabled (0) | SW 검증 비활성화 |
...
│   ─────────────────────────────────                        │
│   • STM32F7 Ethernet MAC이 체크섬 자동 계산/검증           │
│   • CPU 부하 감소                                          │
│   • 소프트웨어 체크섬 불필요 (Disabled)                    │
│                                                             │
│   CHECKSUM_BY_HARDWARE = Disabled                          │
│   ─────────────────────────────────                        │
│   • LwIP가 소프트웨어로 체크섬 계산/검증                   │
│   • CPU 부하 증가                                          │
│   • CHECKSUM_GEN_*, CHECKSUM_CHECK_* = Enabled 필요        │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
