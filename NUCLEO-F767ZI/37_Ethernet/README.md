...
| 파라미터 | 값 | 설명 |
|----------|-----|------|
| LWIP_DHCP | **Disabled** | 고정 IP 사용 |
| IP_ADDRESS | 192.168.1.100 | 보드 IP |
| NETMASK_ADDRESS | 255.255.255.0 | 서브넷 마스크 |
| GATEWAY_ADDRESS | 192.168.1.1 | 게이트웨이 |

> 💡 **DHCP 사용 시**: LWIP_DHCP를 Enabled로 변경

#### 4.3 Key Options

| 파라미터 | 값 | 설명 |
|----------|-----|------|
| MEM_SIZE | 10*1024 | Heap 메모리 크기 |
| TCP_MSS | 1460 | TCP Maximum Segment Size |
| TCP_SND_BUF | 4*TCP_MSS | TCP 송신 버퍼 |
| TCP_WND | 4*TCP_MSS | TCP 수신 윈도우 |
| LWIP_TCP | Enabled | TCP 프로토콜 사용 |
| LWIP_UDP | Enabled | UDP 프로토콜 사용 |
| LWIP_ICMP | Enabled | ICMP (Ping) 응답 |

#### 4.4 Checksum Settings (하드웨어 체크섬)
