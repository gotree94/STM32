...
| 파라미터 | 기본값 | 권장값 (변경) | 설명 |
|----------|--------|---------------|------|
| Flash Interface | AXI Interface | AXI Interface (유지) | |
| ART ACCELERATOR | Disabled | **Enabled** | Flash 접근 가속 |
| Instruction Prefetch | Disabled | Disabled (유지) | |
| CPU ICache | Disabled | **Enabled** | 명령어 캐시 |
| CPU DCache | Disabled | **Enabled** | 데이터 캐시 |

> ⚠️ **3개 항목 변경 필요!** 기본값 그대로 사용하면 DSP 성능이 크게 저하됩니다.
...
│   변경해야 할 항목 (3개):                                                   │
│   ─────────────────────                                                    │
│   1. ART ACCELERATOR:    Disabled → Enabled                                │
│   2. CPU ICache:         Disabled → Enabled                                │
│   3. CPU DCache:         Disabled → Enabled                                │
│                                                                             │
│   성능 비교:                                                                │
│   ───────────                                                              │
│   • Cache OFF: 1024-point FFT ~5ms                                         │
│   • Cache ON:  1024-point FFT ~1ms (5배 빠름!)                             │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

**Floating Point Unit (FPU) 설정:**

> 💡 **참고**: STM32CubeIDE에서 FPU는 프로젝트 생성 시 자동으로 활성화됩니다.
> 별도 설정 없이도 single/double precision FPU가 사용 가능합니다.
> 
> 확인 방법: Project → Properties → C/C++ Build → Settings → MCU Settings → Floating-point unit
