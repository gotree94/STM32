...
## 리드 스위치(Reed Switch)란? 
   ...
   * 그렇지 않으면 스위치는 열린 상태를 유지한다. 이 자기장의 원천은 자석이나 강한 전류일 수 있다.
...
<img src="001.png" width="40%">

   * 전자석이나 영구 자석의 자기장은 리드들이 서로 끌어당겨 회로를 닫게 만든다.
   * 다음 장에서는 이 자기장의 방향이 리드 스위치에 어떤 영향을 미치는지 배울 것이다. 자기장이 제거되면 접점은 초기 위치로 돌아가므로 스위치 접점이 열린다.
...
|  <img src="002.png"> |  <img src="002-1.png"> |
|:-------------------------:|:-------------------------:|
| 자석이 리드 스위치와 평행한 경우 <br>이것이 리드 스위치와 자석 사이의 완벽한 구성이다. | 자석이 리드 스위치와 수직인 경우 <br>이 구성에서는 자기장이 리드 스위치 중간에 사각지대(데드 존)를 갖게 된다. |
| 축적된 자기장이 포물선 형태이기 때문이다. <br>문이나 창문용 리드 스위치에서도 이 구성을 찾을 수 있다. | 따라서 자석과 리드 스위치를 <br>수직 구성으로 사용하지 않는 것이 좋다. |
