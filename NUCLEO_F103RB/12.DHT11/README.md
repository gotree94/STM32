...
1-Wire 프로토콜은 단 하나의 데이터 라인(과 그라운드 라인)만을 사용하여 마스터와 슬레이브 간에 양방향 통신을 수행하는 버스 아키텍처입니다.이해를 돕기 위해 전체적인 하드웨어 연결 구성과 통신 데이터의 기본 타이밍(타임슬롯)을 텍스트 다이어그램으로 시각화해 드립니다.1. 하드웨어 연결 구성 (버스 구조)1-Wire는 오픈 드레인(Open-Drain) 또는 오픈 컬렉터 방식을 사용하므로, 데이터 라인이 기본적으로 High(1) 상태를 유지할 수 있도록 반드시 풀업 저항(Pull-up Resistor, 보통 $4.7\text{ k}\Omega$)이 필요합니다.
```Plaintext
    +5V / +3.3V (VCC)
         │
        ┌┴┐
        │R│ Pull-up Resistor (e.g., 4.7 kΩ)
        └┬┘
         ├──────────────────────────────────────────┐ (1-Wire Data Line)
         │                                          │
   ┌─────┴──────┐                             ┌─────┴──────┐
 ...
 ────────┴──────────────────────────────────────────┴──────── (GND)
```
2. 통신 타이밍 및 데이터 비트 (Time Slot)1-Wire 프로토콜은 별도의 클럭(Clock) 라인이 없기 때문에, 마스터가 데이터 라인을 Low(0)로 떨어뜨리는 시점을 기준으로 타이밍을 계산하여 0과 1을 구별합니다.비트 '0' 쓰기 (Write 0 Slot)마스터가 라인을 Low로 떨어뜨린 후, 비트 주기가 끝날 때까지 오랜 시간(보통 $60\,\mu\text{s}$ 이상) 동안 Low 상태를 유지합니다.
```PlaintextHigh
     ──┐
       │
 Low   └───────────────────────────────┘
...
비트 '1' 쓰기 (Write 1 Slot)마스터가 라인을 아주 잠깐만 Low로 떨어뜨린 후(보통 $1 \sim 15\,\mu\text{s}$), 즉시 라인을 릴리즈(Release)하여 풀업 저항에 의해 다시 High 상태로 복귀하게 만듭니다.
```PlaintextHigh
     ──┐       ┌────────────────────────
       │       │
 Low   └───────┘
...
3. 전체 통신 흐름 (Initialization)DHT11 같은 센서와 통신을 시작할 때 발생하는 초기화 및 응답(Presence) 시퀀스입니다.
```Plaintext
Master Start                                  Slave Response
       (Low for >18ms)                               (Low 80μs -> High 80μs)
High ──┐               ┌───────────────┐             ┌───────────────┐
       │               │               │             │               │
 Low   └───────────────┘               └─────────────┘               └────── Data...
       |<--- >18ms --->|<-- 20~40μs -->|<-- 80μs --->|<-- 80μs ---->|
```

Master Start: 마스터가 라인을 최소 $18\,\text{ms}$ 동안 Low로 당겨서 센서에 준비 신호를 보냅니다.Master Release: 마스터가 라인을 놓고 $20 \sim 40\,\mu\text{s}$ 동안 대기합니다.Slave Response: DHT11 센서가 응답으로 라인을 $80\,\mu\text{s}$ 동안 Low로 만들었다가, 다시 $80\,\mu\text{s}$ 동안 High로 만듭니다.Data Transfer: 이 시퀀스가 끝나면 센서가 본격적으로 40비트(습도 16비트 + 온도 16비트 + 체크섬 8비트)의 데이터를 마스터로 전송하기 시작합니다.
