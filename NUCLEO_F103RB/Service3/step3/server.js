...
// ─── DroidCam Proxy ────────────────────────────────────────────
// /api/camera/stream?ip=192.168.x.x  →  http://<ip>:4747/video  (MJPEG stream)
app.get('/api/camera/stream', (req, res) => {
  const ip = req.query.ip;
  if (!ip) return res.status(400).json({ error: 'IP query parameter is required' });

  const droidUrl = `http://${ip}:4747/video`;

  droidUrl.startsWith('https')
    ? https.get(droidUrl, (droidRes) => pipeStream(droidRes, res))
    : http.get(droidUrl, (droidRes) => pipeStream(droidRes, res));

  function pipeStream(droidRes, clientRes) {
    if (droidRes.statusCode !== 200) {
      clientRes.status(502).json({ error: `DroidCam returned ${droidRes.statusCode}` });
      return;
    }
    clientRes.writeHead(200, {
      'Content-Type': droidRes.headers['content-type'] || 'multipart/x-mixed-replace; boundary=frame',
      'Cache-Control': 'no-cache',
      'Connection': 'close',
      'Access-Control-Allow-Origin': '*',
    });
    droidRes.pipe(clientRes);
    droidRes.on('error', () => clientRes.end());
    clientRes.on('close', () => droidRes.destroy());
  }
});
...
(async () => {
  ...
  https.createServer(creds, app).listen(HTTPS_PORT, () => {
    ...
    console.log(`iPhone DroidCam: 위 주소로 접속 후 IP 입력`);
  });
})();
