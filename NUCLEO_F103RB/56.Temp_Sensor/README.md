...
* 구간별 해석:
   ...
   * ③ 데이터 버스트 (Read/Write Time Slot 구간) — 프레즌스 펄스 이후 buseu가 다시 High로 복귀했다가, 연속된 좁은 펄스들이 이어집니다.  <br>
   이는 1바이트(혹은 그 이상) 단위로 ROM 명령이나 데이터를 주고받는 비트 타임슬롯들로, 각 슬롯마다 짧게 Low로 떨어졌다가 비트 값(0/1)에 따라 Low 유지시간이 달라지는 패턴입니다. <br>
...
* 참고로 정확한 비트값(0/1)을 읽으려면 ③ 구간을 더 짧은 타임베이스(예: 50~100μs/div)로 확대 촬영해서 각 슬롯의 Low 구간 길이를 측정해야 합니다 <br>
