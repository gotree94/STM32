...
* 구간별 해석:
   * ① 리셋 펄스 (Reset Pulse) <br>
     : 마스터(MCU)가 버스를 강제로 Low로 끌어내리는 구간입니다. <br>
     화면상 약 약 600 ~ 650μs 동안 Low를 유지하고 있어, <br>
     DS18B20 규격상 최소 480μs 리셋 요구조건을 만족합니다.
   * ② 프레즌스 펄스 (Presence Pulse) <br>
     : 마스터가 버스를 놓아준 뒤(High), <br>
     센서가 짧게 자신을 Low로 끌어내려 "내가 여기 있다"고 응답하는 구간입니다.  <br>
     폭이 좁은 한 번의 Low 딥(dip)으로 보이며, 규격상 60~240μs 범위(딜레이 15~60μs 후 시작)에 해당합니다.
   * ③ 데이터 버스트 (Read/Write Time Slot 구간) <br>
     : 프레즌스 펄스 이후 buseu가 다시 High로 복귀했다가, 연속된 좁은 펄스들이 이어집니다.  <br>
   ...
   * ④ 유휴 상태 (Idle High) <br>
     : 통신이 끝나고 풀업 저항에 의해 버스가 다시 High로 복귀한 상태입니다. <br>
